...
class Practice_adapter: RecyclerView.Adapter<Practice_adapter.Practice_VH>() {
    ...
    override fun onBindViewHolder(holder: Practice_VH, position: Int) {
        holder.bind(practiceList[position])
    }
    ...
    class Practice_VH(item : View) : RecyclerView.ViewHolder(item) {
        ...
        fun bind(practice: Practice) = with(binding){
            tvTaskTitle.text = practice.title
            if (practice.isCompletedTest && practice.isCompletedPractice) pracCW.strokeColor = ContextCompat.getColor(itemView.context, R.color.green)
